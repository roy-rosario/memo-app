...
function CompleteComponent({data}){
            ...
            <TaskContainer theme={data.theme}>
                    <h2>Tasks</h2>
                        ...
                        <TaskWindow theme={data.theme}>
                            {data.matchResult && data.completedTasks.length > 0 && 
                            <Last 
                                theme={data.theme} 
                            ...
                            {(!data.matchResult && data.completedTasks.length > 0) &&
                                data.completedTasks.slice(data.itemsVisited, data.itemsVisited + data.itemsPerPage)
                                .map(entry => {
                                    return(
                            ...
                            {data.completedTasks.length > 0? data.completedTasks.map((entry) => {

                                    
                              if(data.matchResult){

                                  if(entry === data.completedTasks[data.currentCard]){
                                         ...
                                         <IconHolder>
                                             <CompleteIcon theme={data.theme} onClick={()=>{data.completeTask(entry)}}>
                                              ...
                                              <DeleteIcon
                                                   theme={data.theme}
                                                   onClick={()=>{
                                                        if(data.currentCard === data.completedTasks.length -1){
                            ...
                            {data.matchResult && data.completedTasks.length > 0 && <Next 
                                theme={data.theme} 
                                onClick={data.flipNext} 
                                disabled={data.currentCard === data.completedTasks.length-1}
                                onMouseDown={()=>{data.setBig(true)}} 
                                onMouseUp={()=>{data.setBig(false)}}
                                scaling={data.big}
                            >
